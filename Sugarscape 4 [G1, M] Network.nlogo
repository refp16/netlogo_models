...
to turtle-link ;; turtle procedure
  ;; remove links from previous move
  ask my-out-links [die]
  ;; create links with new neighbors (Von Neumann neighborhood)
  create-links-to turtles-on neighbors4 [set color blue]
end
...
to update-plots
  ...
  if neighbors? [
    set-current-plot "Average neighbors"
    plotxy ticks (count links / count turtles) ;[number-links] of links 
  ] 
end
...
WHAT IS IT?
-----------
This model implements Epstein & Axtell's Sugarscape model under rules ({G1},{M}) and a Social Network Graph, as described in chapter 2 of their book Growing Artificial Societies: Social Science from the Bottom Up. It simulates a population with limited, spatially-distributed resources available. Note this is the same model as Sugarscape 2 [G1, M], only that the possibility of drawing links from each agent to its neighbors is added.
...
The Social network Graph is constructed as a directed graph with agents as the nodes and edges drawn to the agents who have been their neighbors (we will be using the Von Neumann neighborhood). The first agent executes M, moves to a new site and then builds a list of its neighbors, which it mantains until its next move. The second agent does the same and so on. At this point, lines are drawn from each agent to all agents on its list.
...
The five plots show the world population over time, the distribution of sugar among the agents, the mean vision of all surviving agents over time, the mean metabolism of all surviving agents over time and the average number of neighbors for the whole population.
...
See also how the neighbor networks change over time as agents move around on teh Sugarscape. Note that some of the neighbor graphs are simple, while others are elaborate webs containing cycles and other structures.
...
- Ferrer, R. (2011). NetLogo Sugarscape 4 Network Model. Modeling the Social Sciences at http://hardsciences.wordpress.com  
